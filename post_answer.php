...
error_reporting(0);
if(isset($_GET['id'])){
$qry = $conn->query("SELECT t.*,u.name FROM questions t inner join users u on u.id = t.user_id where t.id= ".$_GET['id']);
	...
	<div class="col-lg-12">
		<div class="card">
			<div class="card-body">
				...
				<?php if(count($tags) > 0): ?>
				<div>
					<span class="badge badge-default"><i class="fa fa-tags"></i> Question Category: </span>
		...
		<div class="card">
    		...
    		<div class="col-lg-12">
    			<div class="row">
    				<h3><b> <i class="fa fa-comments"></i> Your Answer(s)</b></h3>
    			</div>
...
<script>
	...
	function submit_reply_func(){
		$('#manage-reply').submit(function(e){
			...
			$.ajax({
				url:'ajax_call.php?action=save_reply',
				method:'POST',
				data:$(this).serialize(),
				success:function(resp){
					if(resp == 1){
						alert_toast("Answer successfully posted.",'success')
						setTimeout(function(){
							location.reload()
						},1000)
					}
	...
	$('#manage-comment').submit(function(e){
		...
		$.ajax({
			url:'ajax_call.php?action=save_comment',
			method:'POST',
			data:$(this).serialize(),
			success:function(resp){
				if(resp == 1){
    ...
    function delete_comment($id){
        start_load()
        $.ajax({
            url:'ajax_call.php?action=delete_comment',
            method:'POST',
            data:{id:$id},
            success:function(resp){
                if(resp==1){
    ...
    function delete_reply($id){
        start_load()
        $.ajax({
            url:'ajax_call.php?action=delete_reply',
            method:'POST',
            data:{id:$id},
            success:function(resp){
                if(resp==1){
